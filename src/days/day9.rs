...
unsafe fn part1_inner(inp: &[u8]) -> u64 {
    let (mut nums, mut empty) = load_u64(inp);
    let mut n_gauss: u64;
...
unsafe fn part2_inner_opt(inp: &[u8]) -> u64 {
    ...
    'outer: for e_ind in (1..=nums.len() - 1).rev() {
        ...
        if let Some((gap_idx, _)) = gaps[e_num - 1..]
            ...
            .max_by_key(|(_, &g)| g)// Max because of the Reverse
        {
        ...
        let num_b = c_cnt.get_unchecked(e_ind) - 1; // zeroes are skipped, so all have something in front.
        let m = e_num as u64;
        ans += (m * (2 * num_b + m + 1) / 2 ) * e_ind as u64;
    }
    ans
